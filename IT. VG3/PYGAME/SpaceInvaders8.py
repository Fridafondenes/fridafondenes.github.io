...
class Karakter(Sprite):
    ...
    def shoot(self):
        ...
        skudd.add(Skudd(x_midpoint, self.y, 0, -10, 5))
...
class Romvesen(Sprite):
    ...
    def flytt(self):
        ...
        if self.y > VINDU_HOYDE:
            ...
            if livsteller == 0:
                fortsett = False  # Avslutter spillet om spilleren har 0 liv
...
class Skudd(Sprite):
    ...
    def treffer_romvesen(self, romvesen):
        """Metode for å sjekke om skuddet treffer romvesenet (boks)"""
        return (
            self.x < romvesen.x + 70
            and self.x + 5 > romvesen.x
            and self.y < romvesen.y + 70
